...
export default function FollowingPage() {
  ...
  const [followingUsers, setFollowingUsers] = useState<FiregramUser[]>([])
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const followingUsersData: FiregramUser[] = []
      
      for (const userId of followingIds) {
        if (users[userId]) {
          followingUsersData.push({ uid: userId, ...users[userId] })
        }
      }
      ...
      if (userProfile && !isOwnProfile) {
        await checkFollowStatus(followingUsersData)
      }
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">
              {isOwnProfile ? 'People you follow' : `People ${profileUser.username} follows`}
            </p>
          </div>
        </div>
